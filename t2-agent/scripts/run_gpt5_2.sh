#!/usr/bin/env bash
# Entrypoint for the GPT-5.2 backend on the ReMMDBench eval server.
#
# Usage:
#   bash scripts/run_gpt5_2.sh                # run full ReMMDBench (10 workers, resume)
#   bash scripts/run_gpt5_2.sh --smoke        # single-sample smoke test
#   bash scripts/run_gpt5_2.sh --limit 20     # run first 20 samples
#   bash scripts/run_gpt5_2.sh --indices 0,5,10
#   bash scripts/run_gpt5_2.sh --no-resume    # force a fresh run directory

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd)"
PROJECT_ROOT="$(cd "${SCRIPT_DIR}/.." >/dev/null 2>&1 && pwd)"
CONFIG_FILE="${PROJECT_ROOT}/configs/gpt5_2.yaml"
